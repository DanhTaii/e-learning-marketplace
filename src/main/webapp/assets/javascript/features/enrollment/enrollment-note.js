                        ...
                        <div class="note-card-header">
                            ...
                            <div class="note-actions">
                                <button class="note-action-btn edit" title="Sửa ghi chú" 
                                    ...
                                    <i class="note-action-icon fa-solid fa-pen-to-square"></i>
                                </button>
                                <button class="note-action-btn delete" title="Xóa ghi chú" 
                                        data-delete-id="${note.id}" data-action="delete">
                                    <i class="note-action-icon fa-solid fa-trash-can"></i>
                                </button>
                            </div>
                        </div>
...
function initNoteAction({noteContainer}) {
    ...
    noteContainer.addEventListener('click', (event) => {
        ...
        if (editBtn) {
            event.stopPropagation();
            const noteId = Number(editBtn.dataset.editId);

            // Đóng các noteCard đang ở chế độ edit
            const openedCancelBtns = document.querySelectorAll('.btn-cancel-edit');
            openedCancelBtns.forEach(btn => btn.click());

            //Tìm card được tác động
            const noteCard = editBtn.closest('.note-card');
            //Tìm thẻ div chứa content
            const contentDiv = noteCard.querySelector('.note-card-content');
            //Lấy nội dung
            const currentContent = contentDiv.innerText;

            triggerInlineEditNote(noteId, noteCard, currentContent)
        }
...
function triggerInlineEditNote(noteId, noteCard, currentContent) {
    const noteCardContentDiv = noteCard.querySelector('.note-card-content')
    const noteActionDiv = noteCard.querySelector('.note-actions')
    ...
    noteCardContentDiv.innerHTML = `
        ...
        <div class="edit-note-actions">
            ...
            <button type="button" class="btn-edit-note btn-save-edit">Cập nhật</button>
        </div>
    `;

    const btnCancel = noteCardContentDiv.querySelector('.btn-cancel-edit');
    btnCancel.addEventListener('click', (e) => {
        e.stopPropagation()
        noteCardContentDiv.innerHTML = currentContent
        noteActionDiv.style.display = 'flex';
    })

    const btnSave = noteCardContentDiv.querySelector('.btn-save-edit');
    btnSave.addEventListener('click', async (e) => {
        e.stopPropagation();

        const newContent = noteCardContentDiv.querySelector(`#edit-textarea-${noteId}`).value.trim();
...
async function updateNote(noteId, newContent) {
    try {
        ...
        if (!response.ok) {
            toast({
                title: 'Thất bại!',
                message: 'Có lỗi xảy ra, không thể lưu ghi chú!',
                type: 'error',
                duration: 4000
            });
        }
