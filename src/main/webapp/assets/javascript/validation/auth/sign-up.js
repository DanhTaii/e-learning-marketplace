...
$('#myForm').on('submit', function (e) {
    ...
    if (usernameError) {
        setError('#error_username', usernameError);
        isValid = false;
    } else {
...
$('#name').on('input', function () {
    ...
    let err = Validator.checkUsername(val);
    err ? setError('#error_username', err) : clearError('#error_username');
});
...
$(document).ready(function () {
    ...
    $('.toggle-password').on('click', function() {
        ...
        // Chuyển loại input
        const type = input.attr('type') === 'password' ? 'text' : 'password'; input.attr('type', type);
        // Đổi icon
